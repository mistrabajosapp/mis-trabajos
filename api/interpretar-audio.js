...
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://iqdtobtborwnytkcexff.supabase.co';
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || 'sb_publishable_xCRCmCF_huD2ed0D0IelvQ_WP1xM5Lg';
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const tiposPermitidos = new Set(['audio/webm', 'audio/mp4', 'audio/ogg', 'audio/mpeg', 'audio/wav']);
...
En observaciones incluí solamente detalles adicionales que no pertenezcan a los demás campos.`;
              ...
              text: {
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    trabajo: { type: ['string', 'null'], description: 'Descripción breve del trabajo.' },
                    costo: { type: ['number', 'null'], minimum: 0 },
                    precio: { type: ['number', 'null'], minimum: 0 },
                    sena: { type: ['number', 'null'], minimum: 0 },
                    fecha_entrega: { type: ['string', 'null'], description: 'Fecha YYYY-MM-DD.' },
                    observaciones: { type: 'string' }
                  },
                  required: ['cliente', 'trabajo', 'costo', 'precio', 'sena', 'fecha_entrega', 'observaciones']
                }
              }
            }
          }
    ...
    if (!geminiResponse.ok) {
      console.error('Gemini API error:', geminiResponse.status, geminiBody?.error?.message);
      ...
      return response.status(geminiResponse.status === 429 ? 429 : 502).json({ error: mensaje });
    }

    const texto = geminiBody?.candidates?.[0]?.content?.parts?.map(part => part.text || '').join('') || '';
    if (!texto) return response.status(502).json({ error: 'Gemini no devolvió datos del trabajo.' });
    ...
    return response.status(status).json({
      error: status === 401 ? 'Tu sesión venció. Volvé a iniciar sesión.' : 'No se pudo procesar el audio. Intentá nuevamente.'
    });
  }
};

async function validarSesion(authorization) {
  if (!authorization?.startsWith('Bearer ')) throw crearError(401, 'Sesión ausente.');
  const authResponse = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
    headers: { Authorization: authorization, apikey: SUPABASE_ANON_KEY }
  });
  if (!authResponse.ok) throw crearError(401, 'Sesión inválida.');
}

function normalizarTrabajo(valor) {
  const numero = dato => Number.isFinite(Number(dato)) && Number(dato) >= 0 ? Number(dato) : null;
  const texto = dato => typeof dato === 'string' && dato.trim() ? dato.trim().slice(0, 500) : null;
  const fecha = /^\d{4}-\d{2}-\d{2}$/.test(valor?.fecha_entrega || '') ? valor.fecha_entrega : null;
  return {
    ...
    fecha_entrega: fecha,
    observaciones: typeof valor?.observaciones === 'string' ? valor.observaciones.trim().slice(0, 1000) : ''
  };
}
